...
const AllProducts = () => {
    ...
    const deleteProd = async (id) => {
        try {
            const res = await fetch(`http://localhost:4000/api/v1/delproduct/${id}`, {
                method: "DELETE",
            });
    ...
    const getAllProducts = async () => {
        try {
            const res = await fetch(
                `http://localhost:4000/api/v1/getproducts?page=${currentPage}&pageSize=${pageSize}&sort=${sort}&search=${searchTerm}`
            );
            if (res.ok) {
